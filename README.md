...
[ekioplot](https://github.com/viniciusoike/ekioplot), which is the
single source of truth for EKIO’s visual identity. Charts and tables
therefore share one palette by construction — `ekiotable` contains no
hex codes of its own.
...
To change a brand color, edit `inst/ekio-palettes.yaml` in `ekioplot`
and rerun its `data-raw/palettes.R`. This package picks the change up
