test_that("gt_theme_ekio_alt() is an internal gt theme", {
  out <- gt_theme_ekio_alt(small_tbl())

  expect_s3_class(out, "gt_tbl")
  expect_equal("gt_theme_ekio_alt" %in% getNamespaceExports("ekiotable"), FALSE)
})

test_that("gt_theme_ekio_alt() validates its inputs", {
  expect_snapshot(gt_theme_ekio_alt(mtcars), error = TRUE)
  expect_snapshot(
    gt_theme_ekio_alt(small_tbl(), table_width = 100),
    error = TRUE
  )
  expect_snapshot(gt_theme_ekio_alt(small_tbl(), font_size = 0), error = TRUE)
  expect_snapshot(gt_theme_ekio_alt(small_tbl(), stripe = "yes"), error = TRUE)
})

test_that("gt_theme_ekio_alt() uses ekioplot palettes", {
  out <- gt_theme_ekio_alt(
    small_tbl(),
    table_width = "80%",
    font_size = 12,
    stripe = FALSE
...
test_that("gt_theme_ekio_alt() rejects a non-string font family", {
  local_font_options()
  expect_snapshot(
...
test_that("the three body levels get distinct styles", {
  local_font_options()
  out <- gt_theme_ekio_alt(grouped_tbl())
...
test_that("the grand summary gets a dark slab with contrasting text", {
  ...
  expect_equal(sum(is.na(cells$colname)), 1L)
  expect_equal(
    unique(vapply(cells$styles, function(s) as_hex(s$cell_text$color), character(1))),
...
test_that("a table with no summaries still gets every other style", {
  ...
  expect_equal(
    all(c("columns_columns", "title", "stub", "row_groups") %in%
      out[["_styles"]]$locname),
    TRUE
  )
})
...
test_that("numeric cells get tabular figures", {
  local_font_options()
  html <- as.character(gt::as_raw_html(gt_theme_ekio_alt(small_tbl())))

  expect_equal(grepl("tabular-nums", html, fixed = TRUE), TRUE)
})

test_that("the fonts resolve like the main theme's", {
  local_font_options()
  out <- gt_theme_ekio_alt(small_tbl())
  ...
  slab <- gt_theme_ekio_alt(small_tbl(), font_body = "roboto_slab")
  expect_equal(
...
test_that("the numeric font inherits the body font and respects overrides", {
  ...
  out <- gt_theme_ekio_alt(tbl, font_body = "Georgia")
  expect_equal(tail(gt_styles_at(out, "data"), 1)[[1]]$cell_text$font, "Georgia")
