...
`gt_theme_ekio()` uses filled blue column headers and spanners, pale blue
group bands, and a dark grand total. It adds an EKIO footer by default.
...
## EKIO alternative (experimental)

The alternative retains the blue header band and uses unfilled group headings
with Baltic Blue rules. It does not add a footer. This function is internal;
the `:::` calls below are for previewing it and are not a stable public API.

::: {data-bs-theme="light"}

```{r alternative-simple}
simple_table |> ekiotable:::gt_theme_ekio_alt(stripe = TRUE)
```
...
```{r alternative-complete}
complete_table |> ekiotable:::gt_theme_ekio_alt(stripe = TRUE)
```
